...
import { addDocumentMaster, updateDocumentMaster } from '@store/actions/masteruser'
...
const MasterForm = (props) => {
    ...
    const onSubmit = data => {
        ...
        if (props && props.preloadValue) { 
          dispatch(updateDocumentMaster(data, props))
        } else {
          dispatch(addDocumentMaster(data, props))
        }
      }
...
if (searchstream.length > 0) {
  const newstring = new RegExp(searchstream)
       const searchitem = userdetail && userdetail.data && userdetail.data.data && userdetail.data.data.filter(item => {
         console.log(item)
         return item.document_name.match(searchstream) 
       })
  ...
  const userdetailapi = 'http://172.105.49.15:3030/api/master/list_document_master'
  useEffect(() => {
    ...
    <div className='auth-wrapper auth-v1 px-2'>
      ...
      { addform === true ?   <Card className='mb-0'>
        ...
        <Card className='mb-0'>
          ...
          <Row>
              <Col md={3}>
              <h4 style={{margin:'10px'}} className='mb-1'>Document List</h4>
              </Col>
              <Col md={2}>
          ...
          <i class="fa fa-minus"></i> Add Document
              </Button.Ripple>
