...
import { addDocumentTypetMaster, updateDocumentTypetMaster } from '@store/actions/masteruser'
...
const columns = [
  ...
  {
    id: 'use_specific_document_type',
    label: 'use_specific_document_type',
    minWidth: 170,
    align: 'right',
    format: (value) => value.toLocaleString('en-US')
  },
...
const MasterForm = (props) => {
    ...
    const onSubmit = data => {
        ...
        if (props && props.preloadValue) { 
          dispatch(updateDocumentTypetMaster(data, props))
        } else {
          dispatch(addDocumentTypetMaster(data, props))
        }
      }
 ...
 <Form className='auth-login-form mt-2' onSubmit={handleSubmit(onSubmit)}>
            ...
            <FormGroup>
                ...
                name="document_type_name"
                  className={classnames({ 'is-invalid': errors['document_type_name'] })}
                  innerRef={register({ required: true, validate: value => value !== '' })}
                 placeholder='Enter your document type' autoFocus />
              </FormGroup>
      {errors.document_type_name && errors.document_type_name.type === 'required' && <p style={{color:"red"}}>document_type_name is required</p>}
              </Col>
              <Col md={4}>
            <FormGroup>
                <Label className='form-label' for='login-email'>
                ...
                <Input
                 ...
                 id='document_name'
                name="document_name"
                  className={classnames({ 'is-invalid': errors['document_name'] })}
                  innerRef={register({ required: true, validate: value => value !== '' })}
                 placeholder='Enter your document name' autoFocus />
              </FormGroup>
      {errors.document_name && errors.document_name.type === 'required' && <p style={{color:"red"}}>document_name is required</p>}
              </Col>
              <Col md={4}>
            <FormGroup>
                <Label className='form-label' for='login-email'>
                ...
                <Input
                 ...
                 id='days'
                name="days"
                  className={classnames({ 'is-invalid': errors['days'] })}
                  innerRef={register({ required: true, validate: value => value !== '' })}
                 placeholder='Enter your require days' autoFocus />
              </FormGroup>
      {errors.days && errors.days.type === 'required' && <p style={{color:"red"}}>days is required</p>}
              </Col>
            <Col md={4}>
              <FormGroup>
        <Label for="exampleText" >use_specific_document_type</Label>
        <Input type="select" name="use_specific_document_type"
                 className={classnames({ 'is-invalid': errors['use_specific_document_type'] })}
                 innerRef={register({ required: true, validate: value => value !== '' })}
        id="exampleSelect">
      ...
      {errors.use_specific_document_type && errors.use_specific_document_type.type === 'required' && <p style={{color:"red"}}>use_specific_document_type is required</p>}
      </Col>
      <Col md={8}>
...
const DocumentTypeMaster = (props) => {
...
if (searchstream.length > 0) {
  const newstring = new RegExp(searchstream)
       const searchitem = userdetail && userdetail.data && userdetail.data.data && userdetail.data.data.filter(item => {
         console.log(item)
         return item.document_name.match(searchstream) || item.document_type_name.match(searchstream) 
       })
  ...
  const userdetailapi = 'http://172.105.49.15:3030/api/master/list_document_type_master'
  useEffect(() => {
    ...
    <div className='auth-wrapper auth-v1 px-2'>
      ...
      { addform === true ?   <Card className='mb-0'>
        ...
        <Card className='mb-0'>
          ...
          <Row>
              <Col md={3}>
              <h4 style={{margin:'10px'}} className='mb-1'>Document Type List</h4>
              </Col>
              <Col md={2}>
          ...
          <i class="fa fa-minus"></i> Add Document Type
              </Button.Ripple>
              ...
              return (
                <TableRow hover role="checkbox" tabIndex={-1}>
                    ...
                    {index + 1}
                      ...
                      <TableCell>
                      {detail.use_specific_document_type === true ? <Switch
        checked={true}
        color="primary"
        name="checkedB"
