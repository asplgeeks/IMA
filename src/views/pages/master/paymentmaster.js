...
import { addPaymentMaster, updatePaymentMaster } from '@store/actions/masteruser'
    ...
    const onSubmit = (data, e) => {
      e.preventDefault()
        if (props && props.preloadValue) { 
          dispatch(updatePaymentMaster(data, props))
        } else {
          dispatch(addPaymentMaster(data, props))
        }
      }
...
<Form className='auth-login-form mt-2' onSubmit={handleSubmit((data, e) => onSubmit(data, e))}>
            ...
            <FormGroup>
                ...
                <Input
                 ...
                 id='Payment_type'
                name="Payment_type"
                  className={classnames({ 'is-invalid': errors['Payment_type'] })}
                  innerRef={register({ required: true, validate: value => value !== '' })}
                 placeholder='Enter payment type' autoFocus />
              </FormGroup>
              {errors.Payment_type && errors.Payment_type.type === 'required' && <p style={{color:"red"}}>Payment type is required</p>}
              </Col>
              <Col md={4}>
              <FormGroup>
                <Label className='form-label' for='login-email'>
...
if (searchstream.length > 3) {
  const newstring = new RegExp(searchstream, 'gi')
       const searchitem = userdetail && userdetail.data && userdetail.data.data && userdetail.data.data.filter(item => {
         return item.Payment_type.match(newstring) 
       })
  ...
  const userdetailapi = 'http://172.105.49.15:3030/api/master/get_payment_master'
  useEffect(() => {
