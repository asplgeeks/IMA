...
import { addStatusMaster, updateStatusMaster } from '@store/actions/masteruser'
...
const MasterForm = (props) => {
    ...
    const onSubmit = data => {
        ...
        if (props && props.preloadValue) { 
          dispatch(updateStatusMaster(data, props))
        } else {
          dispatch(addStatusMaster(data, props))
        }
      }
 ...
 <Form className='auth-login-form mt-2' onSubmit={handleSubmit(onSubmit)}>
            ...
            <FormGroup>
                ...
                <Input
                 ...
                 id='code'
                name="enter_status"
                  className={classnames({ 'is-invalid': errors['enter_status'] })}
                  innerRef={register({ required: true, validate: value => value !== '' })}
                 placeholder='Enter your status' autoFocus />
              </FormGroup>
      {errors.enter_status && errors.enter_status.type === 'required' && <p style={{color:"red"}}>status is required</p>}
              </Col>
...
if (searchstream.length > 0) {
  const newstring = new RegExp(searchstream)
       const searchitem = userdetail && userdetail.data && userdetail.data.data && userdetail.data.data.filter(item => {
         console.log(item)
         return item.enter_status.match(searchstream) || item.status.toString().match(newstring) 
       })
  ...
  const userdetailapi = 'http://172.105.49.15:3030/api/master/list_status'
  useEffect(() => {
    ...
    <div className='auth-wrapper auth-v1 px-2'>
      ...
      { addform === true ?   <Card className='mb-0'>
        ...
        <Card className='mb-0'>
          ...
          <Row>
              <Col md={3}>
              <h4 style={{margin:'10px'}} className='mb-1'>Status List</h4>
              </Col>
              <Col md={2}>
          ...
          <i class="fa fa-minus"></i> Add Status
              </Button.Ripple>
