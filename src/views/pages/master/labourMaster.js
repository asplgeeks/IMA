...
import { addLabourMaster, updateLabourMaster } from '@store/actions/masteruser'
...
const MasterForm = (props) => {
    ...
    const onSubmit = data => {
        ...
        if (props && props.preloadValue) { 
          dispatch(updateLabourMaster(data, props))
        } else {
          dispatch(addLabourMaster(data, props))
        }
      }
 ...
 <Form className='auth-login-form mt-2' onSubmit={handleSubmit(onSubmit)}>
            ...
            <FormGroup>
                ...
                <Input
                 ...
                 id='labour_type'
                name="labour_type"
                  className={classnames({ 'is-invalid': errors['labour_type'] })}
                  innerRef={register({ required: true, validate: value => value !== '' })}
                 placeholder='Enter your labour type' autoFocus />
              </FormGroup>
      {errors.labour_type && errors.labour_type.type === 'required' && <p style={{color:"red"}}>Labour type is required</p>}
              </Col>
              <Col md={4}>
...
if (searchstream.length > 0) {
  const newstring = new RegExp(searchstream)
       const searchitem = userdetail && userdetail.data && userdetail.data.data && userdetail.data.data.filter(item => {
         console.log(item)
         return item.labour_type.match(searchstream)
       })
  ...
  const userdetailapi = 'http://172.105.49.15:3030/api/master/list_labour'
  useEffect(() => {
    ...
    <div className='auth-wrapper auth-v1 px-2'>
      ...
      { addform === true ?   <Card className='mb-0'>
        ...
        <Card className='mb-0'>
          ...
          <Row>
              <Col md={3}>
              <h4 style={{margin:'10px'}} className='mb-1'>Labour List</h4>
              </Col>
              <Col md={2}>
          ...
          <i class="fa fa-minus"></i> Add Labour
              </Button.Ripple>
