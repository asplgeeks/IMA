...
import { addMaterialCategory, updateMaterialCategory } from '@store/actions/masteruser'
...
const MasterForm = (props) => {
    ...
    const onSubmit = data => {
        ...
        if (props && props.preloadValue && props.preloadValue.detail._id) { 
          dispatch(updateMaterialCategory(data, props))
        } else {
          dispatch(addMaterialCategory(data, props))
        }
      }
...
<Form className='auth-login-form mt-2' onSubmit={handleSubmit(onSubmit)}>
            ...
            <FormGroup>
              ...
              <FormGroup>
                <Label className='form-label' for='login-email'>
                Material Catergory Name
                </Label>
                <Input
                 type='text'
                 id='category'
                 name='material_category_name'
                  className={classnames({ 'is-invalid': errors['material_category_name'] })}
                  innerRef={register({ required: true, validate: value => value !== '' })}
                placeholder='Enter catergory name'
                 autoFocus />
              </FormGroup>
              {errors.material_category_name && errors.material_category_name.type === 'required' && <p style={{color:"red"}}>Material catergory name is required</p>}
              </Col>
              <Col md={4}>
              <FormGroup>
                <Label className='form-label' for='login-email'>
                ...
                <Input
                 ...
                 name='material_group_code'
                  className={classnames({ 'is-invalid': errors['material_group_code'] })}
                  innerRef={register({ required: true, validate: value => value !== '' })}
                placeholder='Enter material group code'
                 autoFocus />
              </FormGroup>
              {errors.material_group_code && errors.material_group_code.type === 'required' && <p style={{color:"red"}}>Material group code is required</p>}
              </Col>
            {/* <Col md={4}>
              <FormGroup>
                <Label className='form-label' for='login-email'>
...
if (searchstream.length > 3) {
  const newstring = new RegExp(searchstream, 'gi')
       const searchitem = userdetail && userdetail.data && userdetail.data.data && userdetail.data.data.filter(item => {
         ...
         return item.material_category_name.match(newstring) 
       })
  ...
  const userdetailapi = 'http://172.105.49.15:3030/api/master/list_material_category'
  useEffect(() => {
    ...
    <div className='auth-wrapper auth-v1 px-2'>
      ...
      { addform === true ?   <Card className='mb-0'>
        ...
        <Card className='mb-0'>
          ...
          <Row>
              <Col md={3}>
              <h4 style={{margin:'10px'}} className='mb-1'>Material Catergory List</h4>
              </Col>
              <Col md={2}>
          ...
          <i class="fa fa-plus"></i> Add Material Catergory
          ...
          <i class="fa fa-minus"></i> Add Material Catergory
              </Button.Ripple>
