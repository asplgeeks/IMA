...
import { addFunctionMaster, updateFunctionMaster } from '@store/actions/masteruser'
...
const MasterForm = (props) => {
    const { register, errors, setValue, reset, handleSubmit } = useForm({defaultValues:props ? props && props.preloadValue && props.preloadValue.detail : ""})
    ...
    const onSubmit = (data, e) => {
      e.preventDefault()
        if (props && props.preloadValue) { 
          dispatch(updateFunctionMaster(data, props))
        } else {
          dispatch(addFunctionMaster(data, props))
        }
      }
...
<Form className='auth-login-form mt-2' onSubmit={handleSubmit((data, e) => onSubmit(data, e))}>
            ...
            <FormGroup>
                ...
                <Input
                 ...
                 id='Function_name'
                name="Function_name"
                  className={classnames({ 'is-invalid': errors['Function_name'] })}
                  innerRef={register({ required: true, validate: value => value !== '' })}
                 placeholder='Enter your function name' autoFocus />
              </FormGroup>
      {errors.Function_name && errors.Function_name.type === 'required' && <p style={{color:"red"}}>Function name name  is required</p>}
...
if (searchstream.length > 3) {
  const newstring = new RegExp(searchstream, 'gi')
       const searchitem = userdetail && userdetail.data && userdetail.data.data && userdetail.data.data.filter(item => {
         return item.Function_name.match(newstring) 
       })
  ...
  const userdetailapi = 'http://172.105.49.15:3030/api/master/list_Function_Master'
  useEffect(() => {
    ...
    <div className='auth-wrapper auth-v1 px-2'>
      ...
      { addform === true ?   <Card className='mb-0'>
        ...
        <Card className='mb-0'>
          ...
          <Row>
              <Col md={3}>
              <h4 style={{margin:'10px'}} className='mb-1'>Functional master List</h4>
              </Col>
              <Col md={2}>
          ...
          <i class="fa fa-plus"></i> Add Functional
          ...
          <i class="fa fa-minus"></i> Add Functional
              </Button.Ripple>
