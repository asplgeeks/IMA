...
import { addCostCenter, updateCostCenter } from '@store/actions/masteruser'
...
const MasterForm = (props) => {
    const { register, errors, reset, setValue, handleSubmit } = useForm({defaultValues:props ? props && props.preloadValue && props.preloadValue.detail : ""})
    const [departmentid, setDepartment] = useState('')
    const dispatch = useDispatch()
    
    const departmentmaster = 'http://172.105.49.15:3030/api/master/get_department_list'
    useEffect(() => {
      axios.get(departmentmaster, {
        headers: {
          token:loginAuthID
        }
      })
    ...
    useEffect(() => {
      setValue("department_id", props.preloadValue && props.preloadValue.detail.department_id._id)
    })

    const onSubmit = (data, e) => {
      e.preventDefault()
        if (props && props.preloadValue) { 
          dispatch(updateCostCenter(data, props))
        } else {
          dispatch(addCostCenter(data, props))
        }
      }
...
<Form className='auth-login-form mt-2' onSubmit={handleSubmit((data, e) => onSubmit(data, e))}>
            ...
            <FormGroup>
                ...
                name="cost_center_name"
                  className={classnames({ 'is-invalid': errors['cost_center_name'] })}
                  innerRef={register({ required: true, validate: value => value !== '' })}
                 placeholder='Enter center name' autoFocus />
              </FormGroup>
      {errors.cost_center_name && errors.cost_center_name.type === 'required' && <p style={{color:"red"}}>Cost center name  is required</p>}
              ...
              <FormGroup>
                ...
                <Input
                 ...
                 name='department_id'
                  ...
                  innerRef={register({ required: true, validate: value => value !== '' })}
                 autoFocus >
    {departmentid && departmentid.data && departmentid.data.data && departmentid.data.data.map((depart, index) => {
           return (
...
if (searchstream.length > 3) {
  const newstring = new RegExp(searchstream, 'gi')
       const searchitem = userdetail && userdetail.data && userdetail.data.data && userdetail.data.data.filter(item => {
         return item.cost_center_name.match(newstring) 
       })
  ...
  const userdetailapi = 'http://172.105.49.15:3030/api/master/list_cost_center'
  useEffect(() => {
    ...
    <div className='auth-wrapper auth-v1 px-2'>
      ...
      { addform === true ?   <Card className='mb-0'>
        ...
        <Card className='mb-0'>
          ...
          <Row>
              <Col md={3}>
              <h4 style={{margin:'10px'}} className='mb-1'>Cost Center Master List</h4>
              </Col>
              <Col md={2}>
          ...
          <i class="fa fa-minus"></i> Add Cost
              </Button.Ripple>
              ...
              return (
                <TableRow hover role="checkbox" tabIndex={-1}>
                    ...
                    {index + 1}
                      ...
                      <TableCell>
                      {detail.department_id === null ? 'not found' : detail.department_id.department_name }
                      </TableCell>
                      <TableCell>
